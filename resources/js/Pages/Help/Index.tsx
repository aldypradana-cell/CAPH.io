...
import { 
    MagnifyingGlass as Search, CaretDown as ChevronDown, ChatCircle as MessageCircle, 
    EnvelopeSimple as Mail, Question as HelpCircle, FileText, ShieldCheck as ShieldQuestion, User 
            ...
            <div className="space-y-8 animate-fade-in-up">
                ...
                <div className="relative bg-gradient-to-r from-emerald-600 to-teal-700 rounded-[2.5rem] p-10 text-center text-white overflow-hidden shadow-xl ring-1 ring-[#C5A059]/30">
                    ...
                    <div className="relative z-10 max-w-2xl mx-auto">
                        ...
                        <p className="text-emerald-100 mb-8">Temukan jawaban cepat untuk pertanyaan umum atau hubungi tim support kami.</p>
                        <div className="relative group">
                            <Search weight="bold" className="absolute left-4 top-1/2 transform -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-600 transition-colors" />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1 space-y-4">
                        ...
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 hover:border-emerald-200 dark:hover:border-emerald-800 transition-all group cursor-pointer">
                            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl w-fit mb-4 group-hover:scale-110 transition-transform"><Mail weight="duotone" className="w-6 h-6" /></div>
                            ...
                            <a href="mailto:support@caph.io" className="text-sm font-bold text-emerald-600 dark:text-emerald-400 hover:underline">support@caph.io</a>
                        </div>
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 hover:border-emerald-200 dark:hover:border-emerald-800 transition-all group cursor-pointer">
                            <div className="p-3 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-xl w-fit mb-4 group-hover:scale-110 transition-transform"><MessageCircle weight="duotone" className="w-6 h-6" /></div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="flex items-center justify-between px-2">
                            ...
                            {searchTerm && <span className="text-xs font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-1 rounded-lg">Hasil pencarian: "{searchTerm}"</span>}
                        </div>
                        ...
                        <div className="space-y-4">
                            {filteredFaqs.length === 0 ? (
                                <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800">
                                    <HelpCircle weight="duotone" className="w-12 h-12 text-slate-300 mx-auto mb-3" />
                                    <p className="text-slate-500 dark:text-slate-400 font-medium">Tidak ada hasil ditemukan.</p>
                                    <button onClick={() => setSearchTerm('')} className="text-sm text-emerald-600 font-bold mt-2 hover:underline">Hapus pencarian</button>
                                </div>
                            ) : (
                                filteredFaqs.map((faq, index) => (
                                    <div key={index} className={`bg-white dark:bg-slate-900 rounded-2xl border transition-all duration-300 overflow-hidden ${openIndex === index ? 'border-emerald-200 dark:border-emerald-800 shadow-md' : 'border-slate-200 dark:border-slate-800 shadow-sm hover:border-emerald-100'}`}>
                                        <button onClick={() => setOpenIndex(openIndex === index ? null : index)} className="w-full flex items-center justify-between p-5 text-left focus:outline-none">
                                            <div className="flex items-center gap-4">
                                                <div className={`p-2 rounded-lg ${openIndex === index ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
                                                    {(() => { const Icon = categories.find(c => c.id === faq.category)?.icon || HelpCircle; return <Icon weight="duotone" className="w-5 h-5" />; })()}
                                                </div>
                                                <span className={`font-bold text-sm ${openIndex === index ? 'text-emerald-900 dark:text-white' : 'text-slate-700 dark:text-slate-300'}`}>{faq.question}</span>
                                            </div>
                                            <ChevronDown weight="bold" className={`w-5 h-5 text-slate-400 transition-transform duration-300 ${openIndex === index ? 'rotate-180 text-emerald-500' : ''}`} />
                                        </button>
